...
export default function Dashboard({ reservas, setReservas }) {
  ...
  // Proteção extra: expulsa quem tentar acessar sem login
  if (!user) {
    navigate('/login');
    return null;
  }

  // Função que cria o evento
  const solicitarReserva = (e) => {
    e.preventDefault();
    ...
    setReservas([...reservas, novaSolicitacao]);
  ...
  // Função do funcionário para aprovar
  const aprovarReserva = (id) => {
    setReservas(reservas.map(r => r.id === id ? { ...r, status: 'aprovado' } : r));
  };
